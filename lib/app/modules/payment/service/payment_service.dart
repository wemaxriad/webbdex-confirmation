...
class PaymentService {
  ...
  Future<WithdrawHistoryData?> getPaymentHistoryData({int page = 1}) async {
    final response =
    await server.getRequestToken(endPoint: ApiList.withdrawRequest!+'?page=${page.toString()}');
    final jsonResponse = json.decode(response!.body);
    print(jsonResponse);
    if (response != null && response.statusCode == 200) {
      final jsonResponse = json.decode(response.body);
      final model = WithdrawRequestModel.fromJson(jsonResponse);
      return model.data;
    }
