...
class MyOrdersView extends GetView<MyOrdersController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: primaryColor,
        title: Text(Get.find<GlobalController>().t("My Orders")),
        centerTitle: true,
      ),
      body: Obx(() {
        if (controller.allOrders.isEmpty) {
          return Center(child: Text(Get.find<GlobalController>().t("No orders found")));
        }
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            Get.find<GlobalController>().t(order!.status.toString()),
                            style: TextStyle(
                              color: _statusColor(order!.status.toString()),
                            ),
                          ),
  ...
  Future<void> openDialPad(String phone) async {
    ...
    if (!await launchUrl(
      ...
      Get.snackbar(
          globalController.t("Error"), globalController.t("Could not open dial pad"));
    }
  }
